...
(function () {
  ...
  kintone.events.on('app.record.index.show', (event) => {
    try {
      ...
      // 念のため：ビュー側で絞る想定だが、JSでも保険で絞る
      const recs = (event.records || []).filter(r => {
        const op = r[FIELD.OP]?.value;
        return !FIELD.OP ? true : (op === SHIP_VALUE);
      });
      ...
      title.textContent = `出荷状況（出荷先別）  ※この一覧の表示分で集計（${recs.length}件）`;
      title.style.fontWeight = '700';
      title.style.margin = '6px 0 10px';
      box.appendChild(title);

      const rows = aggregate(recs);
      box.appendChild(renderTable(rows));
    } catch (e) {
      console.error('[SHIP_SUMMARY_VIEW] failed', e);
    }
    return event;
  });
  ...
  function aggregate(records) {
    ...
    // date降順が理想（ビュー側で設定してね）
    for (const r of records) {
      const dest = (r[FIELD.DEST]?.value ?? '').trim() || '(未設定)';
      const date = r[FIELD.DATE]?.value || '';
      const qty = num(r[FIELD.QTY]?.value);
      const species = (r[FIELD.SPECIES]?.value ?? '').trim();
      ...
      if (!row.lastDate && d) row.lastDate = d;
    ...
    const total = rows.reduce((acc, r) => acc + r.fyTotal, 0);
    rows.push({ dest: '合計', fyTotal: total, lastDate: '', lastQty: '', lastSpecies: '' });
  ...
  function renderTable(rows) {
    ...
    ['出荷先一覧', '今年度の累計', '直近の出荷日', '直近の出荷量', '出荷樹種'].forEach(t => {
  ...
  function getOrCreateBox() {
    ...
    let box = document.getElementById('ws-ship-summary-view-box');
    if (!box) {
      box = document.createElement('div');
      box.id = 'ws-ship-summary-view-box';
  ...
  function fmt(v) { const n = Math.round(Number(v) * 10) / 10; return `${n}`; }
